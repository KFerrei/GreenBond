    ...
    var body: some View {
        TimelineView(.animation()) { timeline in
                Canvas { context, size in
                    ...
                    let angle1 = cos(Angle.degrees(now.remainder(dividingBy: 3)*15).radians)
                    ...
                    path.addCurve(to: CGPoint(x: 0.1563*w*angle2, y: 0.6391*h*angle1), control1: CGPoint(x: 0.3848*w*angle1, y: h*angle2), control2: CGPoint(x: 0.3126*w*angle1, y: 0.7944*h*angle2))
